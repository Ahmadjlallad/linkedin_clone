...
function Post({ post, modalPost }: Props) {
  ...
  const [m, setModalOpen] = useRecoilState(modalState);
  const truncate = (inputStr: string, numberOfLetters: number): string =>
    inputStr.length > numberOfLetters
      ? inputStr.substring(0, numberOfLetters) + "...see more"
      : inputStr;
  function deletePost() {
    ...
    >
      <div className="flex items-center px-2.5 cursor-pointer">
        ...
        <div className="mr-auto ml-2 leading-none">
          <h6 className="font-medium hover:text-blue-500 hover:underline-offset-8">
            {post.name}
          </h6>
